...
def pulp_fiction_actors
  ...
  Movie.select('actors.*').joins(:actors).where('title = ?', 'Pulp Fiction')
end
...
def uma_movies
  ...
  Actor.select('movies.id, movies.title, movies.yr').joins(:movies).where('name = ?', 'Uma Thurman').order('movies.yr ASC')
end
